  ...
  <div class="clearfix selector">
    <div class="type-wrap logo">
      ...
      <div class="value logos">
        <ul class="logo-list">
          <li v-for="tm in trademarkList" :key="tm.tmId" @click="attrInfo(tm)">{{tm.tmName}}</li>
          <!-- <li>索尼（SONY）</li>
          <li>TCL</li>
          <li>长虹（CHANGHONG）</li>
          <li>飞利浦（PHILIPS）</li>
    ...
    <div class="type-wrap" v-for="attrs in attrsList" :key="attrs.attrId">
      ...
      <div class="fl value">
        <ul class="type-list">
          <li v-for="(list,index) in attrs.attrValueList" :key="index" @click.once="attrsListInfo(attrs,list)">
            <a>{{list}}</a>
          </li>
        </ul>
      </div>
  ...
  export default {
    ...
    methods: {
      attrInfo(tm){
        ...
        // this.$router.push({
        //   name: "search",
        //   params: { keyword: tm.tmName || undefined }
        // })
      },
      attrsListInfo(attrs,list){
        //["属性ID:属性值:属性名"]
        this.$emit('attrsInfo',attrs,list)
      }
    },
  }
</script>
